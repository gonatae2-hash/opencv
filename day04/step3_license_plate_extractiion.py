...
def find_license_plate(img):  
    ...
    roi = img[int(height*0.4):height, 0:int(width*0.5)]
    # 1️⃣ 전처리
    ...
    for cnt in contours:
        ...
        if 2000 < area < 2500:
            ...
            print(f"면적:{area:.0f} 비율:{aspect_ratio:.1f}")
    #         
    #         # 번호판: 가로가 세로의 3~5배
            if 3 < aspect_ratio < 5.5:
    ...
    #     
        ...
        plate = cv.warpPerspective(roi, M, (w, h))
    #     
        offset_y = int(height * 0.4)
        return plate, (x, y + offset_y, w, h)
    #
    return None, None
